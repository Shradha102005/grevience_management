...
function Grievances() {
  ...
  const fetchGrievances = useCallback(async () => {
    try {
      ...
      if (isStaff) {
        try {
          const { data: statsData } = await api.get<GrievanceStats>(
            "/api/municipal/stats",
          );
          setStats(statsData);
        } catch {
...
<<<<<<< HEAD
      ...
      >
        <div className="flex items-center gap-4">
          <div
            className="h-12 w-12 rounded-2xl flex items-center justify-center text-rose-600 shadow-xl shadow-rose-500/10"
            style={{
...
<<<<<<< HEAD
                    ...
                    style={{
                      animation: `slideIn 0.3s cubic-bezier(0.16,1,0.3,1) ${i * 0.04}s both`,
                    }}
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-2.5">
                        ...
                        <span
                          className={`text-[10px] font-extrabold uppercase tracking-widest px-2.5 py-0.5 rounded-md border ${statusColors[r.status] || "bg-slate-100 text-slate-600 border-slate-200"}`}
                        >
                          {r.status}
                        </span>
                      ...
                      <h4 className="font-extrabold text-slate-800 text-lg leading-tight truncate group-hover:text-rose-600 transition-colors">
                        {r.title}
                      </h4>
                              ...
                              <div className="relative" key={idx}>
                                ...
                                <p className="text-xs text-slate-900">
                                  <span className="font-black">
                                    {h.changed_by_name ?? "Officer"}
                                  </span>{" "}
                                  changed status to{" "}
                                  <span
                                    className={`font-bold px-1.5 py-0.5 rounded text-[10px] ${statusColors[h.new_status] ?? "bg-slate-100 text-slate-600"}`}
                                  >
                                    {h.new_status}
                                  </span>
                                </p>
