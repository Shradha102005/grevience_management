    ...
    try {
      const res = await fetch(`${API_BASE}/live/schemes/check-eligibility`, {
        ...
        body: JSON.stringify({
          ...
          land_hectares: form.land_hectares
            ? parseFloat(form.land_hectares)
            : null,
        }),
      });
        ...
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white/80 backdrop-blur-xl z-10">
          <div>
            <h2 className="text-xl font-extrabold text-slate-900 flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-indigo-500" /> Auto-Eligibility
              Match
            </h2>
            <p className="text-xs text-slate-500 font-semibold mt-1">
              AI analyzes your profile to find schemes you qualify for.
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          {!checked ? (
            <div className="space-y-6">
              ...
              <Button
                onClick={handleCheck}
                disabled={loading}
                className="w-full h-12 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold shadow-lg shadow-indigo-500/20 text-base"
              >
                {loading ? (
                  <>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between bg-indigo-50 border border-indigo-100 p-4 rounded-xl">
                <p className="text-sm text-indigo-900 font-medium">
                  <span className="font-extrabold text-indigo-700 text-lg mr-1">
                    {results.length}
                  </span>{" "}
                  schemes matched your profile.
                </p>
                <button
              ...
              <div className="space-y-4">
                {results.map(({ scheme, score, reasons }, i) => (
                  <div
                    key={i}
                    className="bg-white border border-slate-100 shadow-sm rounded-2xl p-5 relative overflow-hidden group hover:border-indigo-200 transition-colors"
                  >
                    ...
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <span className="text-xs font-black text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100 shrink-0">
                        {score}% Match
                      </span>
                    </div>
                    <div className="mt-4 flex flex-wrap gap-2">
                      {reasons.slice(0, 3).map((r: string, j: number) => (
                        <span
                          key={j}
                          className="text-[10px] font-bold uppercase tracking-wider text-slate-500 bg-slate-50 px-2 py-1 rounded-md border border-slate-100"
                        >
                          ✓ {r}
                        </span>
                      ))}
                    ...
                    {(scheme.apply_url || scheme.portal_url) && (
                      <a
                        href={scheme.apply_url || scheme.portal_url}
                        target="_blank"
...
function SchemeAI() {
  ...
  const fetchSchemes = useCallback(
    async (q = "", cat = "", p = 1, isLoadMore = false) => {
      setLoading(true);
      try {
        ...
        setScrapeInfo((prev) => ({
          ...prev,
          source: data.source || "myscheme.gov.in",
        }));
      } catch {
  ...
  const handleSearch = (val: string) => {
    ...
    searchTimeout.current = setTimeout(
      () => fetchSchemes(val, activeCategory, 1, false),
      300,
    );
  };
      ...
      {!selected && (
        ...
        >
          <div className="flex items-center gap-4">
            <div
              className="h-12 w-12 rounded-2xl flex items-center justify-center text-indigo-600 shadow-xl shadow-indigo-500/10"
              style={{
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                {scrapeInfo?.source === "myscheme.gov.in" &&
                  !scrapeInfo?.is_scraping && (
          ...
          <div className="flex items-center gap-3">
            <div className="relative shadow-sm rounded-xl">
              ...
              <Input
                className="h-11 pl-10 pr-10 text-sm w-72 bg-white/70 border-white shadow-sm text-slate-800 focus:border-indigo-300 focus:ring-4 focus:ring-indigo-500/10 rounded-xl transition-all placeholder:text-slate-400"
                placeholder="Search by name, ministry, tags..."
                value={search}
            ...
            <Button
              ...
              className="h-11 bg-white hover:bg-slate-50 text-indigo-600 border border-white shadow-sm font-bold px-4 ml-2 rounded-xl transition-transform hover:-translate-y-0.5"
            >
            ...
            <Button
              ...
              className={`h-11 font-bold px-4 rounded-xl shadow-lg transition-transform hover:-translate-y-0.5 ${chatOpen ? "bg-slate-800 hover:bg-slate-900 text-white shadow-slate-900/20" : "bg-indigo-600 hover:bg-indigo-700 text-white shadow-indigo-500/20"}`}
            >
              {chatOpen ? (
                <>
                  <X className="mr-2 h-4 w-4" /> Close AI
                </>
              ) : (
                <>
                  <Bot className="mr-2 h-4 w-4" /> Scheme Assistant
                </>
              )}
            </Button>
          </div>
      ...
      <div className="flex-1 flex overflow-hidden p-3 justify-center z-10 relative">
        ...
        {!selected && (
          <div className="w-full max-w-7xl h-full flex flex-col">
            ...
            <div
              className="flex-1 overflow-y-auto space-y-4 pr-2 pb-10"
              style={{ scrollbarWidth: "none" }}
            >
              {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-5">
                  {schemes.map((scheme, i) => (
                    <div
                      ...
                      style={{
                        animation: `slideIn 0.3s cubic-bezier(0.16,1,0.3,1) ${i * 0.05}s both`,
                      }}
                    >
                      <div>
                        <div className="flex items-start justify-between gap-4 mb-4">
                          <span
                            className={`text-[10px] font-extrabold uppercase tracking-widest px-3 py-1 rounded-lg border ${getCategoryPill(scheme.category)}`}
                          >
                            {scheme.category}
                          </span>
                        ...
                        <h4 className="font-extrabold text-slate-800 text-xl leading-tight mb-2 group-hover:text-indigo-600 transition-colors line-clamp-2">
                          {scheme.name}
                        </h4>
                      ...
                      <div className="mt-5 pt-5 border-t border-slate-200/50 flex items-center justify-between">
                        <div className="flex flex-col">
                          {scheme.ministry && (
                            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1 line-clamp-1">
                              {scheme.ministry}
                            </span>
                          )}
                          {scheme.tags && scheme.tags.length > 0 && (
                            <div className="flex gap-2">
                              {scheme.tags.slice(0, 2).map((tag, idx) => (
                                <span
                                  key={idx}
                                  className="text-[10px] bg-white border border-slate-200 text-slate-500 px-2 py-0.5 rounded-md font-semibold"
                                >
                                  {tag}
                                </span>
                              ))}
              ...
              {hasMore && (
                <div className="flex justify-center mt-6 mb-4">
                  <Button
                    onClick={loadMore}
                    disabled={loading}
                    className="bg-white hover:bg-slate-50 text-indigo-600 border border-slate-200 shadow-sm font-bold px-6 rounded-xl transition-all hover:-translate-y-0.5"
                  >
                    {loading ? (
        ...
        {selected && (
          <div className="w-full max-w-full h-full flex flex-col bg-white/80 backdrop-blur-3xl border border-white shadow-2xl shadow-slate-300/60 rounded-[2rem] overflow-hidden animate-in zoom-in-95 duration-300">
            <div
              className={`h-1.5 w-full ${CATEGORY_DOT[selected.category] || "bg-slate-300"}`}
            />
            ...
            <div className="px-6 py-4 border-b border-slate-100/80 bg-white/60 flex items-center gap-4 shrink-0">
              <Button
                ...
                className="h-9 w-9 bg-white hover:bg-slate-50 border border-slate-100 text-slate-600 rounded-full shadow-sm shrink-0 transition-transform hover:scale-105"
              >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span
                    className={`text-[10px] font-extrabold uppercase tracking-widest px-2.5 py-0.5 rounded-md border ${getCategoryPill(selected.category)}`}
                  >
                    {selected.category}
                  </span>
                ...
                <h2 className="text-2xl font-black text-slate-900 tracking-tight leading-snug line-clamp-2">
                  {selected.name}
                </h2>
                {selected.ministry && (
            ...
            >
              <div className="space-y-8">
                ...
                {(selected.benefit || selected.description) && (
                  <div>
                    <p className="text-[11px] font-black uppercase tracking-widest text-indigo-500 mb-4 flex items-center gap-1.5">
                      <Sparkles className="w-3 h-3" /> Benefits & Details
                    </p>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selected.tags.map((tag, i) => (
                        <span
                          key={i}
                          className="px-3 py-1.5 text-xs font-semibold rounded-lg border border-slate-200 bg-slate-50 text-slate-600"
                        >
                          {tag}
                        </span>
                      ))}
                ...
                {selected.eligibility && (
                  <div className="bg-slate-900 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                      <p className="text-[11px] font-black uppercase tracking-widest text-indigo-400 mb-4 flex items-center gap-2">
                        Who is Eligible?
                      </p>
                      <p className="text-base font-medium leading-relaxed whitespace-pre-wrap text-slate-200">
                        {selected.eligibility}
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-slate-100">
                  ...
                  {selected.launched_year && (
                    <span className="flex items-center gap-1.5 text-xs text-slate-400 font-bold ml-auto">
                      <Clock className="w-3.5 h-3.5 text-indigo-400" /> Est.{" "}
                      {selected.launched_year}
                    </span>
                  )}
                  {selected.helpline && (
        ...
        <div className="fixed bottom-6 right-6 w-[28rem] z-50 bg-white/90 backdrop-blur-2xl shadow-2xl shadow-slate-300/60 rounded-3xl overflow-hidden border border-white animate-in slide-in-from-bottom-8 duration-300">
          <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-slate-900 to-slate-800 text-white">
            <span className="flex items-center gap-3 font-bold text-lg">
              <Bot className="w-5 h-5 text-indigo-400" /> Scheme Assistant
            </span>
            <button
