...
const trafficColor = (v: number) =>
  v > 75 ? "#f43f5e" : v > 50 ? "#fbbf24" : "#34d399";
    ...
    : aqi <= 100
      ...
      : aqi <= 200
        ...
        : aqi <= 300
          ? "#fb923c"
          : "#f43f5e";
const parkingColor = (pct: number) =>
  pct > 80 ? "#f43f5e" : pct > 60 ? "#fbbf24" : "#34d399";
=======
//  Types 
...
<<<<<<< HEAD
      ...
      >
        ...
        >
          ...
          >
            As a government officer, your portal is locked to{" "}
            <strong style={{ color: "#a78bfa" }}>{lockedCity}</strong>.
=======
...
<<<<<<< HEAD
            ...
            >
              Nation-wide smart city portal — pick any Indian city
            </p>
=======
...
>>>>>>> 4b6b11d5b8430477f7a10a0fb94cf381a9b34171
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "assistant",
      content: `Hi! I'm your Smart City assistant for **${city}**. Ask me about traffic, transport, weather, nearby services, or any city-related query! 🏙️`,
    },
  ]);
...
<<<<<<< HEAD
              ...
              <p style={{ fontSize: "10px", color: "#a78bfa", margin: 0 }}>
                {city} · {LANGUAGES.find((l) => l.code === language)?.label}
              </p>
=======
...
function SmartCity() {
  ...
  const fetchData = useCallback(
    async (tabOverride?: string) => {
      ...
      if (tab === "Overview" || tab === "Weather") {
        setL("weather", true);
        fetch(`${API_BASE}/smart-city/weather?city=${encodeURIComponent(city)}`)
          .then((r) => r.json())
          .then(setWeather)
          .catch(() => {})
          .finally(() => setL("weather", false));
      }
      if (tab === "Overview" || tab === "Traffic") {
        setL("traffic", true);
        fetch(`${API_BASE}/smart-city/traffic?city=${encodeURIComponent(city)}`)
          .then((r) => r.json())
          .then(setTraffic)
          .catch(() => {})
          .finally(() => setL("traffic", false));
      }
      if (tab === "Transport") {
        setL("transport", true);
        fetch(
          `${API_BASE}/smart-city/transport?city=${encodeURIComponent(city)}`,
        )
          .then((r) => r.json())
          .then(setTransport)
          .catch(() => {})
          .finally(() => setL("transport", false));
      }
      if (tab === "Nearby") {
        setL("nearby", true);
        fetch(
          `${API_BASE}/smart-city/nearby?city=${encodeURIComponent(city)}&service_type=${nearbyType}`,
        )
          .then((r) => r.json())
          .then((d) => setNearby(d.services || []))
          .catch(() => {})
          .finally(() => setL("nearby", false));
      }
      if (tab === "Parking") {
        setL("parking", true);
        fetch(`${API_BASE}/smart-city/parking?city=${encodeURIComponent(city)}`)
          .then((r) => r.json())
          .then((d) => {
            setParking(d.parking || []);
            setParkingAsOf(d.as_of || null);
          })
          .catch(() => {})
          .finally(() => setL("parking", false));
      }
      if (tab === "Events") {
        setL("events", true);
        fetch(`${API_BASE}/smart-city/events?city=${encodeURIComponent(city)}`)
          .then((r) => r.json())
          .then((d) => setEvents(d.events || []))
          .catch(() => {})
          .finally(() => setL("events", false));
      }
      if (tab === "Overview") {
        fetch(`${API_BASE}/live/city-telemetry`)
          .then((r) => r.json())
          .then(setTelemetry)
          .catch(() => {});
      }
  ...
  useEffect(() => {
    if (city && activeTab === "Nearby") fetchData("Nearby");
  }, [nearbyType]);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      if (["Overview", "Traffic", "Weather"].includes(activeTab)) fetchData();
    }, 30000);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      >
        {[
          {
            label: "Temperature",
            value: weather?.current.temp ? `${weather.current.temp}°C` : "—",
            icon: Thermometer,
            color: "#fbbf24",
            sub: weather?.current.description || "Loading...",
          },
          {
            label: "AQI",
            value: weather?.aqi.aqi ?? "—",
            icon: Wind,
            color: aqiColor(weather?.aqi.aqi ?? 0),
            sub: weather?.aqi.category || "—",
          },
          {
            label: "High Traffic Zones",
            value: traffic?.high_congestion_zones.length ?? "—",
            icon: TrafficCone,
            color: "#f43f5e",
            sub: "Right now",
          },
          ...
          },
          {
            label: "Transport",
            value: transport
              ? `${transport.modes.filter((m) => m.status === "ok").length}/${transport.modes.length}`
              : "—",
            icon: Bus,
            color: "#a78bfa",
            sub: "Modes on time",
...
<<<<<<< HEAD
              ...
              >
                ...
                >
                  {(traffic?.zones ?? []).map((e, i) => (
                    <Cell key={i} fill={trafficColor(e.index)} />
                  ))}
=======
...
<<<<<<< HEAD
                  ...
                  >
                    ...
                    {new Date(a.posted_at).toLocaleTimeString("en-IN")}
                  </p>
=======
...
<<<<<<< HEAD
            ...
            >
              No active advisories — traffic is flowing normally in {city}
            </p>
=======
...
<<<<<<< HEAD
          ...
          {transport.alerts.map((a: any) => (
            ...
            >
              <strong style={{ color: "#f43f5e" }}>{a.mode}:</strong>{" "}
              {a.message}
=======
...
<<<<<<< HEAD
          ...
          >
            <strong>Transport Helpline:</strong>{" "}
            <span style={{ color: "#60a5fa" }}>{transport.helpline}</span>
=======
...
<<<<<<< HEAD
            <button
              key={t.key}
              onClick={() => setNearbyType(t.key)}
              style={{
                padding: "8px 14px",
                borderRadius: "20px",
                border: `1px solid ${active ? t.color : "rgba(139,92,246,0.15)"}`,
                background: active ? `${t.color}15` : "transparent",
                color: active ? t.color : "var(--color-muted-foreground)",
                fontSize: "12px",
                fontWeight: 700,
                cursor: "pointer",
                display: "flex",
...
<<<<<<< HEAD
          <p style={{ fontSize: "13px", margin: 0 }}>
            No {nearbyType} locations found for {city}
          </p>
=======
...
>>>>>>> 4b6b11d5b8430477f7a10a0fb94cf381a9b34171
        ...
        >
          {nearby.map((svc) => {
            const typeConfig =
              NEARBY_TYPES.find((t) => t.key === svc.type) || NEARBY_TYPES[0];
...
<<<<<<< HEAD
              ...
              >
                ...
                >
                  <Icon
                    style={{ width: 17, height: 17, color: typeConfig.color }}
                  />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {svc.phone && (
                    ...
                    >
                      <Phone
                        style={{ width: 11, height: 11, color: "#60a5fa" }}
                      />
                      <p
                        style={{
                          fontSize: "11px",
...
<<<<<<< HEAD
      ...
      >
        Data from OpenStreetMap contributors · Free & open source
      </p>
=======
...
<<<<<<< HEAD
          ...
          >
            Live Occupancy Status — Auto-refreshing simulation model (As of{" "}
            <strong>{parkingAsOf}</strong>)
=======
          <p style={{ fontSize: "14px", color: "var(--color-foreground)", margin: 0 }}>
...
<<<<<<< HEAD
                ...
                >
                  ...
                  >
                    {spot.available_slots} of {spot.total_slots} available
                  </p>
                  <p
                    style={{
                      fontSize: "11px",
...
<<<<<<< HEAD
              ...
              {canManage && (
                <button
                  onClick={async () => {
                    const val = prompt(
                      `Update available slots (current: ${spot.available_slots}):`,
                    );
...
<<<<<<< HEAD
                ...
                >
                  <p style={{ fontSize: "72px", margin: 0 }}>
                    {weather.current.icon}
                  </p>
=======
              ...
              <div style={{ display: "flex", gap: "16px" }}>
                {[
                  ...
                  },
                  {
                    ...
                    value: weather.aqi.aqi,
                    icon: Activity,
                    color: aqiColor(weather.aqi.aqi),
                  },
              ...
              >
                <div
                  style={{
                    ...
                    height: "70px",
                    borderRadius: "50%",
                    border: `4px solid ${aqiColor(weather.aqi.aqi)}`,
                    display: "flex",
                    flexDirection: "column",
                    alignItems: "center",
                    justifyContent: "center",
                    background: `${aqiColor(weather.aqi.aqi)}12`,
                  }}
                >
                  <p
                    style={{
              ...
              >
                {weather.forecast.slice(0, 5).map((d) => (
                  ...
                  >
                    ...
                    <p
                      style={{ fontSize: "10px", color: "#60a5fa", margin: 0 }}
                    >
                      {d.rain_chance}% 🌧️
                    </p>
=======
...
<<<<<<< HEAD
          <button
            onClick={async () => {
              ...
              const cat = prompt(
                "Category (marathon/festival/road_closure/exhibition/announcement):",
              );
              ...
              const res = await fetch(`${API_BASE}/smart-city/events`, {
                ...
                body: JSON.stringify({
                  ...
                  category: cat,
                  location: loc,
                  event_date: date ? `${date}T00:00:00` : null,
                }),
              });
              fetchData("Events");
            }}
...
>>>>>>> 4b6b11d5b8430477f7a10a0fb94cf381a9b34171
        ...
        >
          {events.map((ev) => {
            const cat =
              EVENT_CATEGORIES.find((c) => c.key === ev.category) ||
              EVENT_CATEGORIES[4];
            return (
<<<<<<< HEAD
              <div
...
<<<<<<< HEAD
                      ...
                      >
                        <MapPin
                          style={{ width: 12, height: 12, color: "#a78bfa" }}
                        />
                        <p
                          style={{
                            fontSize: "11px",
                    ...
                    {ev.event_date && (
                      ...
                      >
                        <Calendar
                          style={{ width: 12, height: 12, color: "#60a5fa" }}
                        />
                        <p
                          style={{
                            fontSize: "11px",
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <div>
            ...
            >
              Nation-wide · AI-powered · Live data
            </p>
          </div>
          {/* Live pill */}
          <div
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <button
            onClick={() => {
              if (!isOfficer) setShowCitySelector(true);
            }}
            style={{
              display: "flex",
              alignItems: "center",
              ...
              fontSize: "13px",
              fontWeight: 700,
              cursor: isOfficer ? "default" : "pointer",
            }}
          >
            ...
            style={{
              ...
              background: isAdmin
                ...
                : isOfficer
                  ? "rgba(96,165,250,0.1)"
                  : "rgba(52,211,153,0.1)",
              color: isAdmin ? "#f43f5e" : isOfficer ? "#60a5fa" : "#34d399",
              border: `1px solid ${isAdmin ? "rgba(244,63,94,0.2)" : isOfficer ? "rgba(96,165,250,0.2)" : "rgba(52,211,153,0.2)"}`,
            }}
          >
        ...
        {TABS.map((tab) => (
          <button
            ...
            style={{
              ...
              color:
                activeTab === tab ? "#a78bfa" : "var(--color-muted-foreground)",
              fontSize: "12px",
